...
深度目录预测会按源图尺寸分组。每个包含 `batch_size` 个兼容张量的完整组只执行一次模型调用；不足完整批次的图像逐张运行。流水线不会对图像做 padding。`workers` 控制并行图像读取和 CPU 预处理，不控制模型推理。
...
缩放会在自定义 `transform` 之前执行。在使用 spawn 的平台上，`workers > 0` 时该变换必须可序列化。
