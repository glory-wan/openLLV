...
公共三通道输入和 NumPy 输出使用 RGB。基类把它转换为 `_enhance()` 所需的三通道 BGR、8 位工作图，并在结束后转回 RGB。DCP 在反相图上估计大气光与透射率，以灰度引导滤波细化透射率，并返回按 `255` 缩放的值。与本组其他算法不同，`_enhance()` 忽略全部运行时关键字；请在构造时或通过 `set_params()` 修改算法值。工厂会忽略不支持的构造键，并在控制台说明其不会被使用且不影响计算；近似拼写会获得建议。
