...
公共三通道输入和 NumPy 输出使用 RGB；NPE 仅把 BGR/BGRA 作为 `_enhance()` 内部布局，并在内部支持灰度图。它会保留灰度/alpha 布局。基类将解析后的源值域映射到 NPE 的 `[0,1]` 工作值域，随后恢复原输入的值域约定与可选 dtype。全部算法参数均可作为经校验的单次 `enhance()` 覆盖值，且不修改已存状态。工厂创建会过滤不支持的构造键并告警。
