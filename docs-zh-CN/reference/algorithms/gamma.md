...
公共三通道输入和 NumPy 输出使用 RGB；基类在 `_enhance()` 边界完成内部 BGR 往返转换。基类会解析输入的语义值域，将其映射到 Gamma 的 `[0,1]` 浮点工作值域，并在处理后恢复相同的源值域与 dtype。因此浮点 `[0,1]` 和浮点 `[0,255]` 输入会分别保持自己的输出约定。传给 `enhance()` 的 `gamma` 仅覆盖本次调用，不修改已存值。小于 `1` 的值提亮图像。工厂会忽略不支持的构造键，并在控制台说明其不会被使用且不影响计算；近似拼写会获得建议。
