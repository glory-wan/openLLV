...
Important options include `--backend`, `--device`, `--output-dir`, `--batch-size`, `--workers`, `--resize`, `--no-save`, `--output-name`, and `--output-ext`. `--resize SIZE` selects a square input; `--resize HEIGHT WIDTH` selects an explicit size. Omitting it performs no scaling.
