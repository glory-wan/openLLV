...
Deep directory inference groups images by source size. Every complete group of `batch_size` compatible tensors uses one model call; incomplete groups run one image at a time. The pipeline never pads images. `workers` controls parallel image reading and CPU preprocessing, not model inference.
...
Resizing happens before a custom `transform`. With `workers > 0` on a spawn-based platform, that transform must be picklable.
