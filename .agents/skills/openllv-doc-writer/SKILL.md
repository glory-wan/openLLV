...
- [ ] 返回值契约写清楚了（单图返回 `(增强结果, Path|None)`；目录 `save=True` 返回路径列表，`save=False` 返回增强图列表）？
...
- **行为契约**：写清"什么输入 → 什么输出"。例如：单图返回 `(image, path)`；单图 `save=False` 时 `path` 为 `None`；目录 `save=True` 返回按路径排序的 `Path` 列表，`save=False` 返回按相同顺序排列的增强图列表且不写文件；`backend="auto"` 时 `.pt`/`.pth` 文件走深度后端。
