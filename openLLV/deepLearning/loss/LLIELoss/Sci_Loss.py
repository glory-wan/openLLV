"""SCI reference-free loss functions."""
...
class Sci_Loss(BaseLoss):
    """Reference-free SCI loss."""
    ...
    def __init__(self):
        """Initialize SCI fidelity and smoothness losses."""
    ...
    def forward(self, input, enhanced):
        ...
        Smooth_Loss = self.smooth_loss(input, enhanced)

        return 1.5 * Fidelity_Loss + Smooth_Loss


class SmoothLoss(nn.Module):
    """SCI weighted smoothness regularization."""

    def __init__(self):
        """Initialize SCI smoothness loss."""
    ...
    def rgb2yCbCr(self, input_im):
        ...
        Args:
            input_im: RGB image tensor with shape ``[B, 3, H, W]``.

        Returns:
            YCbCr image tensor with shape ``[B, 3, H, W]``.
        """
        im_flat = input_im.contiguous().view(-1, 3).float()
        mat = input_im.new_tensor([[0.257, -0.148, 0.439], [0.564, -0.291, -0.368], [0.098, 0.439, -0.071]])
        bias = input_im.new_tensor([16.0 / 255.0, 128.0 / 255.0, 128.0 / 255.0])
